...
Provisions the Gin/Angular app's inventory, source GitHub Secret,
collector/deployer RBAC, Entra registrations,
and ingress NetworkPolicy. The app pipeline deploys `mucsi96/go-app` **1.0.0**
and `mucsi96/client-app` **22.0.0**.

Authentication follows `setup_hello_app` (skeleton-app) and `setup_library_app`:
shared `register_api` and `register_spa`, authorization code + PKCE, bearer API JWTs, `api-access`
...
The Go chart owns the `observatory-api-workload-identity` ServiceAccount, following
the same `<app>-api-workload-identity` convention as the other p07 apps. Its name
is used by the workload federation and read-only collector RoleBindings. It configures
...
First provision the new API/SPA registrations, inventory, deployment role and
collector RoleBindings. Deploy the new application images with the pinned charts,
...
New inputs include `client_log_url`, `k8s_oidc_issuer_url`, and
