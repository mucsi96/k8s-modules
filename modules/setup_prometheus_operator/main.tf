...
# Grafana Faro: a dedicated Alloy instance running as a faro.receiver. The
...
# them as Loki log streams. This is a separate Helm release from the Alloy
# DaemonSet in setup_loki that scrapes node pod logs; the two have different
# lifecycles (one runs per node, one is a single-replica HTTP server).
#
# loki_url is passed as a literal string rather than referenced from
# module.setup_loki to avoid a dependency cycle (setup_loki waits on
# kube-prometheus-stack readiness for its datasource ConfigMap drop).
# faro.receiver buffers and retries until Loki is reachable, so eventual
# readiness is sufficient.
resource "helm_release" "faro_alloy" {
  name       = local.faro_release_name
  ...
  values = [yamlencode({
    fullnameOverride = local.faro_release_name
    ...
    alloy = {
      configMap = {
        ...
        # DaemonSet, so the same Loki query filters span backend pod logs and
        # frontend SPA telemetry.
        content = <<-RIVER
          ...
          loki.write "default" {
            endpoint {
              url = "${var.faro_loki_url}/loki/api/v1/push"
            }
          }
        RIVER
      }
      # Expose the Faro HTTP port through the Service the chart renders so the
      # Traefik IngressRoute below can route to it.
      extraPorts = [{
        name       = "faro"
...
# Lock down var.faro_cors_allowed_origins to specific SPA origins in production.
