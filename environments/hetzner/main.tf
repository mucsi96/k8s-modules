...
module "setup_hetzner_host" {
  source          = "../../modules/setup_hetzner_host"
  server_name     = var.hetzner_server_name
  location        = var.hetzner_location
  server_type     = var.hetzner_server_type
  image           = var.hetzner_image
  ssh_username    = var.hetzner_ssh_username
  ssh_initial_port = 22
}

module "setup_cluster" {
  source                = "../../modules/setup_cluster"
  host                  = module.setup_hetzner_host.host
  initial_port          = module.setup_hetzner_host.initial_port
  username              = module.setup_hetzner_host.username
  initial_password      = module.setup_hetzner_host.initial_password
...
data "azurerm_key_vault_secret" "dns_zone" { key_vault_id = data.azurerm_key_vault.kv.id name = "dns-zone" }
data "azurerm_key_vault_secret" "letsencrypt_email" { key_vault_id = data.azurerm_key_vault.kv.id name = "letsencrypt-email" }
data "azurerm_key_vault_secret" "cloudflare_zone_id" { key_vault_id = data.azurerm_key_vault.kv.id name = "cloudflare-zone-id" }
data "azurerm_key_vault_secret" "cloudflare_account_id" { key_vault_id = data.azurerm_key_vault.kv.id name = "cloudflare-account-id" }
data "azurerm_key_vault_secret" "cloudflare_api_token" { key_vault_id = data.azurerm_key_vault.kv.id name = "cloudflare-api-token" }
data "azurerm_key_vault_secret" "cloudflare_team_domain" { key_vault_id = data.azurerm_key_vault.kv.id name = "cloudflare-team-domain" }
data "azurerm_key_vault_secret" "authorized_as" { key_vault_id = data.azurerm_key_vault.kv.id name = "authorized-as" }
data "azurerm_key_vault_secret" "twingate_api_token" { key_vault_id = data.azurerm_key_vault.kv.id name = "twingate-api-token" }
data "azurerm_key_vault_secret" "twingate_network" { key_vault_id = data.azurerm_key_vault.kv.id name = "twingate-network" }
data "azurerm_key_vault_secret" "github_token" { key_vault_id = data.azurerm_key_vault.kv.id name = "github-token" }

provider "cloudflare" { api_token = data.azurerm_key_vault_secret.cloudflare_api_token.value }
provider "twingate" { api_token = data.azurerm_key_vault_secret.twingate_api_token.value network = data.azurerm_key_vault_secret.twingate_network.value }
provider "github" { owner = "mucsi96" token = data.azurerm_key_vault_secret.github_token.value }
...
module "setup_twingate" {
  ...
  k8s_host           = module.setup_hetzner_host.host
  depends_on         = [module.setup_cluster]
}
...
locals { owner = data.azurerm_client_config.current.object_id }
