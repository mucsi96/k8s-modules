resource "azurerm_key_vault_secret" "hetzner_host" {
  ...
  name         = "hetzner-host"
  value        = module.setup_hetzner_host.host
}

resource "azurerm_key_vault_secret" "hetzner_bootstrap_password" {
  key_vault_id = data.azurerm_key_vault.kv.id
  name         = "hetzner-bootstrap-password"
  value        = module.setup_hetzner_host.initial_password
}
